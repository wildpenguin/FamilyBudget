...
describe("GET /api/transactions", () => {
	...
	it("lists all transactions for the authenticated user's family", async () => {
		...
		expect(
			res.body.data.map((t: { description: string }) => t.description).sort(),
		).toEqual(["Bonus", "Paycheck"]);
	});
});
...
describe("POST /api/transactions", () => {
	...
	it("returns 400 for an invalid payload", async () => {
		...
		const res = await request(app)
			.post("/api/transactions")
			.set("Authorization", `Bearer ${token}`)
			.send({ familyId: family.id, amountCents: "not-a-number" });
...
describe("PUT /api/transactions/:id", () => {
	it("updates a transaction's fields", async () => {
		...
		const res = await request(app)
			...
			.set("Authorization", `Bearer ${token}`)
			.send({ amountCents: 5000, description: "Weekly shop, updated" });
