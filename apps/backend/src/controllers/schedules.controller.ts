...
export const schedulesController = {
    async createSchedule(req: AuthenticatedRequest, res: Response) {
        ...
        const schedule = await schedulesRepository.create(parsedBody.data, req.userId);
    ...
    async deleteSchedule(req: AuthenticatedRequest, res: Response) {
        ...
        const deleted = await schedulesRepository.delete(parsedParams.data.id, member.familyId);
        if (!deleted) {
            return res.status(404).json({ error: "Schedule not found" });
        }
    ...
    async getSchedules(req: AuthenticatedRequest, res: Response) {
        ...
        if (input.data.id) {
            const [schedule] = await schedulesRepository.get(familyMember.familyId, input.data.id);
            if (!schedule) {
                return res.status(404).json({ error: "Schedule not found" });
            }
