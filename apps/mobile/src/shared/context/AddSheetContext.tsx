...
import { Divider, Icon, Text, TouchableRipple, useTheme, type MD3Theme } from 'react-native-paper';
...
export function AddSheetProvider({ children }: AddSheetProviderProps) {
    ...
    return (
        <AddSheetContext.Provider value={{ isOpen, openAddSheet, closeAddSheet }}>
            ...
            <BottomSheet isPresented={isOpen} onDismiss={closeAddSheet} showDragIndicator>
...
function AddSheetRow({ destination, theme, onPress }: AddSheetRowProps) {
    ...
    return (
        <TouchableRipple onPress={onPress} borderless style={styles.row} rippleColor={theme.colors.surfaceVariant}>
...
function getTintColors(tint: Tint, theme: MD3Theme): { background: string; foreground: string } {
    switch (tint) {
        case 'success':
            return { background: '#DCFCE7', foreground: '#16A34A' };
        case 'danger':
            return { background: theme.colors.errorContainer, foreground: theme.colors.error };
        case 'accent':
        default:
            return { background: theme.colors.primaryContainer, foreground: theme.colors.primary };
    }
}
