...
export function CategoryListItem({ category, onDelete, isDeleting }: CategoryListItemProps) {
    ...
    return (
        <View style={styles.row}>
            <View style={[styles.iconCircle, { backgroundColor: tint.background }]}>
                <Icon source={isIncome ? 'arrow-up' : 'arrow-down'} size={16} color={tint.foreground} />
            </View>

            <View style={styles.textContainer}>
                <Text style={{ fontSize: 14, color: theme.colors.onSurface }}>{category.name}</Text>
